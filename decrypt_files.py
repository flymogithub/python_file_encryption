...
from file_decrypt import decrypt_file
...
extension_list = {'.jpg', '.jpeg', '.png', '.mp4', '.mov', '.webm','webp'}
excluded_dirs = {'.cache','.trillian','My Pictures','Documents','Desktop','mozilla','player','Music','lost+found','.mozilla'}
username = getpass.getuser()

logging.basicConfig(filename="/tmp/"+(username)+"decrypt.log",level=logging.INFO,format="%(asctime)s %(levelname)s %(message)s")
...
def decrypt_files(root_dir=None):
    ...
    for path in basedir.rglob('*.enc'):
        logging.info(f"Processing file: {path}")
        if is_excluded(path):
            logging.info(f"Skipping excluded directory: {path}")
            continue
        try:
            logging.info(f"Decrypting file: {path}")
            ...
            logging.error(f"Decryption failed for {path}: Invalid authentication tag.")
            continue
...
def main(dry_run=False, root_dir=None):
    ...
    decrypt_files(root_dir=root_dir)
